...
def add_track(peer_id: int, url: str, type_: str):
    with _lock:
        conn = _conn()
        cur = conn.cursor()
        cur.execute("INSERT OR IGNORE INTO tracks (peer_id, url, type, last_id) VALUES (?, ?, ?, NULL)",
                    (peer_id, url, type_))
        conn.commit()
        conn.close()
...
def add_warn(peer_id: int, user_id: int):
    with _lock:
        conn = _conn()
        cur = conn.cursor()
        cur.execute("INSERT OR IGNORE INTO warns (peer_id, user_id, count) VALUES (?, ?, 0)",
                    (peer_id, user_id))
...
def add_ban(peer_id: int, user_id: int):
    with _lock:
        conn = _conn()
        cur = conn.cursor()
        cur.execute("INSERT OR IGNORE INTO bans (peer_id, user_id) VALUES (?, ?)",
                    (peer_id, user_id))
        conn.commit()
        conn.close()
